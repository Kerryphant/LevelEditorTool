...
void DisplayChunk::TerrainPicking(DirectX::SimpleMath::Ray ray)
{
	//closet intersection needs to be identified to ensure correct one is picked
	
	float distance = 0;
	float smallestDist = 10000000000;
	
	XMFLOAT3 botLeft, topLeft, topRight, botRight;

	std::pair <int, int> vertexIndices[4];
	...
	for (int i = 0; i < TERRAINRESOLUTION - 1; i++)
	{
		...
		{
			//current quad
			botLeft = m_terrainGeometry[i][j].position;
			topLeft = m_terrainGeometry[i][j+1].position;
			topRight = m_terrainGeometry[i+1][j+1].position;
			botRight = m_terrainGeometry[i+1][j].position;

			//if there is an intersection in the first triangle
			if (ray.Intersects(botLeft, topRight, topLeft, distance))
			{
				//if the distance is closer to the camera than the previous
				if (distance < smallestDist)
				{
					//update smallest distance
					smallestDist = distance;

					//store index to vertices
					vertexIndices[0] = std::make_pair(i, j);
					vertexIndices[1] = std::make_pair(i, j+1);
					vertexIndices[2] = std::make_pair(i+1, j+1);
					vertexIndices[3] = std::make_pair(i+1, j);
				}
			}
			else if (ray.Intersects(botLeft, botRight, topRight, distance))//if there is an intersection in the second triangle
			{
				if (distance < smallestDist)
				{
					//update smallest distance
	...
	for (int i = 0; i < 4; i++)
	{
		int index = (TERRAINRESOLUTION * vertexIndices[i].first) + vertexIndices[i].second;
		
		BYTE temp = m_heightMap[index];
...
void DisplayChunk::SetLowerOrRaise(bool lower)
{
	m_lower = lower;
}
