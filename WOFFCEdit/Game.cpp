...
int Game::MousePicking()
{
    ...
    for (int i = 0; i < m_displayList.size(); i++)
    {
        ...
        for (int y = 0; y < m_displayList[i].m_model.get()->meshes.size(); y++)
        {
            ...
            {
                if ((lowestDistance == -1) || lowestDistance > pickedDistance)
                {
                    lowestDistance = pickedDistance;
                    selectedID = i;
                }
            }
        }
...
void Game::BuildDisplayList(std::vector<SceneObject> * SceneGraph)
{
    ...
    {
        for (auto i = m_displayList.begin(); i != m_displayList.end();)
        {
            i->m_texture_diffuse->Release();
            i = m_displayList.erase(i);
        }
...
XMFLOAT4 Game::GetXZIntersection()
{
    const XMVECTOR nearSource = XMVectorSet(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 0.0f, 1.0f);
    const XMVECTOR farSource = XMVectorSet(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 1.0f, 1.0f);

    //Unproject the points on the near and far plane, with respect to the matrix we just created.
    XMVECTOR nearPoint = XMVector3Unproject(nearSource, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_camera.GetView(), XMMatrixIdentity());
    XMVECTOR farPoint = XMVector3Unproject(farSource, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_camera.GetView(), XMMatrixIdentity());

    //turn the transformed points into our picking vector. 
    XMVECTOR pickingVector = farPoint - nearPoint;
    pickingVector = XMVector3Normalize(pickingVector);

    XMVECTOR zxPlane = XMVectorSet(0.f, 1.f, 0.f, 0.0f);

    XMVECTOR intersection = DirectX::XMPlaneIntersectLine(zxPlane, farPoint, nearPoint);
    
    XMFLOAT4 intersectionFloat;    //the float where we copy the intersection vector members
    XMStoreFloat4(&intersectionFloat, intersection);   //the function used to copy

    return intersectionFloat;
}

void Game::GetRayValues(DirectX::SimpleMath::Vector3* position, DirectX::SimpleMath::Vector3* direction)
{
    const XMVECTOR nearSource = XMVectorSet(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 0.0f, 1.0f);
    const XMVECTOR farSource = XMVectorSet(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 1.0f, 1.0f);
    ...
    XMFLOAT3 tempDir;
    XMStoreFloat3(&tempDir, pickingVector);
    DirectX::SimpleMath::Vector3 tempDirection(tempDir.x, tempDir.y, tempDir.z);
   
    XMFLOAT3 tempPos;
    XMStoreFloat3(&tempPos, nearPoint);
    DirectX::SimpleMath::Vector3 tempPosition(tempPos.x, tempPos.y, tempPos.z);
...
void Game::TerrainSculpt()
{
    DirectX::SimpleMath::Vector3 directionMouseToPlane;
    DirectX::SimpleMath::Vector3 rayStartPos;

    GetRayValues(&rayStartPos, &directionMouseToPlane);

    //create ray from mouse to intersecton point
    DirectX::SimpleMath::Ray ray = DirectX::SimpleMath::Ray(rayStartPos, directionMouseToPlane);

    m_displayChunk.SetLowerOrRaise(m_terrainModeLower);
    m_displayChunk.TerrainPicking(ray);

}

void Game::SetLowerOrRaise(bool lower)
{
    m_terrainModeLower = lower;
}
